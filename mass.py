...
# Added wraplength to keep this line from changing the column widths - Chris Carmody 21 Sep
weightTotalOutput = Label(root, text="Press the button to perform weight calculation", bg="black", fg="cyan",
...
def weight_calculator():
    ...
    try:
        ...
        if newtons > 500:
            weightTotalOutput.configure(text="Please try again! That is much too heavy!", bg="black", fg="cyan",
                                        font="Helvetica 10")

        # Check if the newtons is less than 100 and write s message to a label
        elif newtons < 100:
            weightTotalOutput.configure(text="Please try again! That is much too light!", bg="black", fg="cyan",
                                        font="Helvetica 10")

        # If its not to heavy or too light, write a newtons to a label
        else:
            weightTotalOutput.configure(text=f"That is juuuuust right at {newtons:.1f} newtons", bg="black", fg="cyan",
                                        font="Helvetica 10")

    # Catch any exception and write to a label
    except:
        weightTotalOutput.configure(text="Invalid Entry. Please enter a number.", bg="yellow",  fg="red",
                                    font="Helvetica 10 bold")


# Configure button to execute weight_calculator() function
buttonToCalculateNewtons.configure(command=weight_calculator)
